	...
	public static BigInteger recoverSecret(BigInteger N, BigInteger X,
		...
		for (int i =0;i<runs.length; i++) {
			for (int j=0;j<runs.length ;j++ ) {
				if(runs[i].R.equals(runs[j].R) && i!=j){
					if (runs[i].c==1) {
						BigInteger invers = runs[j].s.modInverse(N);
						result = runs[i].s.multiply(invers).mod(N);
					}
					else{
						BigInteger invers = runs[i].s.modInverse(N);
						result = runs[j].s.multiply(invers).mod(N);
					}
				}
			}
		}
